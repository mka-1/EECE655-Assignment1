...
#this code was written by mohamad
from scapy.layers.inet import IP, Ether, TCP, ICMP, fragment
from scapy.sendrecv import send, sendp, sniff
from scapy import *

incoming = sniff(filter='IP')  # sniffs packets with TCP and IP layers
...
def detection1(fragments):  # pass fragmentsInput in line 17 to it
    ...
    for fragment in fragments:  # need to know how to access fragment object to see Fragment Offset and Fragment Protocol and Transport Length
        if fragment[IP].frag == 0 and fragment.haslayer(TCP) and len(
                fragment[IP]) < 8:  # Tmin here is 8 by default
...
def detection2(
        fragments):  # check if syn field is different in different fragments (to bypass a firewall for example) (if a syn was sent in the first header, then modified in the second fragment)
    for i in range(0, len(fragments) - 1):  # fragments input is a list of the fragments detected by sniff
        if fragments[i][IP].syn != fragments[i + 1][
            IP].syn:  # compare 2 consecutive fragments, if syn is in both and different, then there is an attack
...
def detection3(fragments):  # against TearDrop attack
    for i in range(0, len(fragments) - 1):  # fragments input is a list of the fragments detected by sniff
        ...
        if ((fragments[i][IP].frag == 0) and (fragments[1][IP].flags == "MF") and (
                fragments[i + 1][IP].flags != "MF") and (
                fragments[i + 1][IP].frag + len(fragments[i + 1][IP]) < len(fragments[i][IP]))):
            print("Possibility of an attack, drop packet")
            return
